...
public abstract class DocKit {
    ...
    public static String forField(Class<?> clazz, String field) {
        ClassDoc doc = getClassDoc(clazz);
    ...
    public static String forConstant(Class<?> clazz, String name) {
        ClassDoc doc = getClassDoc(clazz);
        if (doc == null) return null;
        FieldDoc[] fields = doc.fields();
        for (FieldDoc fd : fields) if (fd.name().equals(name)) return fd.getRawCommentText() != null ? format(fd.getRawCommentText()) : null;
        return null;
    }
    ...
    private static String format(String comment) {
        return comment == null || comment.trim().equals("") ? null : comment.trim();
    }
