 ...
 **/
public class SpringBootInterpreter implements Interpreter {

    @Override
    public ClassInterpretation interpret(Class<?> clazz) {
        ClassDoc doc = SpringBootJavadoc.of(clazz);
    ...
    public MethodInterpretation interpret(Method method) {
        MethodDoc doc = SpringBootJavadoc.of(method);
        if (doc == null) return null;
    ...
    public FieldInterpretation interpret(Field field) {
        FieldDoc doc = SpringBootJavadoc.of(field);
        if (doc == null) return null;
        Tag[] tags = doc.tags();
        Note[] notes = new Note[tags != null ? tags.length : 0];
        for (int i = 0; tags != null && i < tags.length; i++) notes[i] = new Note(tags[i].kind(), tags[i].name(), tags[i].text());
    ...
    public EnumInterpretation interpret(Enum<?> constant) {
        FieldDoc doc = SpringBootJavadoc.of(constant);
        if (doc == null) return null;
        Tag[] tags = doc.tags();
        Note[] notes = new Note[tags != null ? tags.length : 0];
        for (int i = 0; tags != null && i < tags.length; i++) notes[i] = new Note(tags[i].kind(), tags[i].name(), tags[i].text());
    ...
    public static abstract class SpringBootJavadoc {
        private static Logger logger = LoggerFactory.getLogger(SpringBootJavadoc.class);
        ...
        static {
            ...
            if (jarLocations.length > 1) {
                ...
                ClassLoader classLoader = SpringBootJavadoc.class.getClassLoader();
        ...
        private synchronized static ClassDoc getClassDoc(Class<?> clazz) {
            ...
            Main.execute(new String[]{
                    "-doclet",
                    SpringBootJavadoc.class.getName(),
                    "-encoding",
                    "utf-8",
                    "-classpath",
                    libPath,
